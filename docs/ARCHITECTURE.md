...
- Three paths reach the same user-facing manual-entry frame (a "Haven't found what you're looking for?" affordance that lets the user type the domain in directly): (1) zero candidates returned, (2) the Clearbit call itself failed (timeout, rate-limited, network error), and (3) a user-triggered "Can't find what you're looking for?" text link shown beneath a non-empty candidate list. Paths (1) and (2) auto-route; path (3) is opt-in when Clearbit returned ≥1 suggestion the user still rejects. Per the §6 error-translation pattern, (1) and (2) can carry different internal log detail even though the user-facing destination frame is identical.
